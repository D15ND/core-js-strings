...
function isPalindrome(str) {
  const strLower = str.toLowerCase().replace(/[?!, ]/g, '');
  const mid = Math.floor(strLower.length / 2);
  let left = '';
  let right = '';
  for (let i = 0; i < mid; i += 1) {
    left += strLower[i];
  }
  for (let k = strLower.length - 1; k >= strLower.length - mid; k -= 1) {
    right += strLower[k];
  }
  return left === right;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].length > stack[0].length) {
      stack.push(arr[i]);
      stack.shift();
    }
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (word === word.toLowerCase()) {
      creater.push(str[i].toUpperCase());
    } else {
      creater.push(str[i].toLowerCase());
    }
  }
  return creater.join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    let letter = str[i];
    let codeLetter = str.charCodeAt(i);
    if (codeLetter >= 65 && codeLetter <= 90) {
      codeLetter = ((codeLetter - 65 + 13) % 26) + 65;
      letter = String.fromCharCode(codeLetter);
    }
    if (codeLetter >= 97 && codeLetter <= 122) {
      codeLetter = ((codeLetter - 97 + 13) % 26) + 97;
      letter = String.fromCharCode(codeLetter);
    }
    res += letter;
  }
